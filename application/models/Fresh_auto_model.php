...
class Fresh_auto_model extends CI_Model {
	...
	function get_srs_items_po_divisor($from,$to,$items=array(),$offtake_coverage= null){
		$this->db = $this->load->database("default", true);
		$this->db->select("product_history.product_id,'".$offtake_coverage."' as divisor,sum(product_history.selling_area_out) - sum(product_history.wholesale_qty) as total_sales",false);
